"""
Regenerative Cooling Channel Design Tool — DITTUS-BOELERT EQUATION VERSION
RP-1 / H2O2 Bipropellant Thruster — Inconel 718 Wall

Methodology:
  - Cervone AE4903 Lecture 7 (TU Delft): 1D thermal resistance network
  - DITTUS-BOELERT EQUATION: empirical correlation for forced convection
  - Isentropic relations for axial gas conditions
  - Counter-flow coolant energy balance

NOTE: This version uses the Dittus-Boelert correlation for hot gas heat transfer.
      Dittus-Boelert: Nu = 0.023 * Re^0.8 * Pr^0.3
      h_g = 0.023 * (k_g^0.7 * cp_g^0.3) / (D^1.8 * mu_g^0.5) * (4*mdot_total/π)^0.8
      Reference: Classic empirical correlation for turbulent forced convection
...
# ====================================================================
# HOT GAS HEAT TRANSFER COEFFICIENT — DITTUS-BOELERT EQUATION
# ====================================================================

def hot_gas_htc(radius, T_aw):
    """
    Dittus-Boelert correlation for hot gas heat transfer:
    
    Nu = 0.023 * Re^0.8 * Pr^0.3
    h_g = 0.023 * (k_g^0.7 * cp_g^0.3) / (D^1.8 * mu_g^0.5) * (4*mdot_total/π)^0.8
    
    This is a classical empirical correlation for forced convection heat transfer
    in turbulent flow regimes. The Dittus-Boelert equation is one of the most 
    widely used correlations in heat exchanger design.
    
    The mass flow term (4*mdot_total/π)^0.8 ensures proper scaling with local 
    flow conditions across the nozzle.
    """
    h_g = np.zeros(len(radius))
    for i, r in enumerate(radius):
        D   = 2 * r
        # Dittus-Boelert formula: h_g = 0.023 * (k^0.7 * cp^0.3) / (D^1.8 * mu^0.5) * (4*mdot/π)^0.8
        numerator = 0.023 * (k_g**0.7) * (cp_g**0.3)
        denominator = (D**1.8) * (mu_g**0.5)
        mass_term = (4.0 * mdot_total / np.pi)**0.8
        h_g[i] = numerator / denominator * mass_term
    return h_g
...
def plot_results(x, radius, d, n_valid):
    ...
    fig.suptitle(
        ...
        f"RP-1/H2O2, Inconel 718 (DITTUS-BOELERT)  |  Re = {d['Re']:.0f}  V = {d['V']:.1f} m/s",
    ...
    txt = (f"DESIGN SUMMARY (DITTUS-BOELERT)\n{'─'*36}\n"
           f"N channels        : {d['N_ch']}\n"
    ...
    plt.savefig("cooling_design_result_dittus_boelert.png", dpi=150, bbox_inches='tight')
    print("  Saved: cooling_design_result_dittus_boelert.png")
    plt.show()
...
def plot_design_space(valid):
    ...
    ax.set_title("Valid Design Space — Coking Safety Margin (DITTUS-BOELERT)")
    ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig("design_space_dittus_boelert.png", dpi=150, bbox_inches='tight')
    print("  Saved: design_space_dittus_boelert.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    print("  --- DITTUS-BOELERT EQUATION VERSION ---")
    print("=" * 60)
    ...
    # Hot gas HTC — DITTUS-BOELERT EQUATION version
    h_g = hot_gas_htc(radius, T_aw)
    print(f"  h_g  : throat = {h_g[i_t]:.0f} W/m²K  |  chamber = {h_g[0]:.0f} W/m²K")
    print(f"  NOTE: h_g computed using DITTUS-BOELERT equation")
