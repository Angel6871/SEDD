...
Outputs:
    ...
    - plots saved to ./trajectory_plots/
"""
...
# ── CEA run to load ───────────────────────────────────────────────────────────
RUN_ID          = "run_20260223_084141"     # folder name inside ./outputs/
RESULTS_CSV     = f"./outputs/{RUN_ID}/results.csv"

# ── Design point selector — pick the row you want ────────────────────────────
DESIGN_PC_BAR   = 8.3      # [bar]  must match a Pc_bar value in the CSV
DESIGN_OF       = 6.05       # [-]    must match an OF value in the CSV

# ── Mission ───────────────────────────────────────────────────────────────────
TARGET_ALTITUDE_M   = 1000  # [m]
...
def ambient_pressure_pa(h_m: float) -> float:
    ...
    if _USE_AMBIANCE:
        ...
        # Beyond 32 km — approximate exponential decay
        else:
...
def run_trajectory() -> pd.DataFrame:
    ...
    """
    t_max_powered  = burn_time_s * 1.05   # slight margin
